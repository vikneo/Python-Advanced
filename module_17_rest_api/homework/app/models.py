...
def _get_book_obj_from_row(row: tuple) -> Book:
    ...
    return Book(
        id = row[0],
        title = row[1],
        author=Author(id=row[3], first_name=row[4], last_name=row[5], middle_name=row[6])
    )


def _get_book_obj_author(row: tuple) -> Book:
    return Book(id=row[0], title=row[1], author=row[2])
...
def get_author_by_last_name(last_name: str) -> Optional[Author] | bool:
    with sqlite3.connect(DATABASE_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"""
