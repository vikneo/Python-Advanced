...
def decrypt(encryption: str) -> str:
    ...
    while True:
        res = re.search(r'([.]{2}|[.])', encryption)

        if res.end() - res.start() == 2 and re.search(r"\w+", encryption):
            encryption = encryption[:res.start() - 1] + encryption[res.start() + 2:]
        elif re.search(r"\w+", encryption):
            encryption = encryption[:res.start()] + encryption[res.start() + 1:]
            break
        else:
            raise
    return encryption
